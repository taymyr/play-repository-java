...
abstract class JPARepository<Aggregate : Any, Identity : Serializable>
    ...
    constructor(
        ...
        override fun get(id: Identity): CompletionStage<Optional<Aggregate>> = execute { em -> ofNullable(em.find(clazz, id)) }

        override fun getAll(): CompletionStage<List<Aggregate>> =
            executeRO { em ->
        ...
        override fun remove(aggregate: Aggregate): CompletionStage<Done> =
            execute { em ->
                if (em.contains(aggregate)) {
                    em.remove(aggregate)
                } else {
                    em.remove(em.merge(aggregate))
                }
                Done.getInstance()
            }

        override fun removeAll(aggregates: Collection<Aggregate>): CompletionStage<Done> =
            execute { em ->
                aggregates.forEach {
                    if (em.contains(it)) {
                        em.remove(it)
        ...
        override fun create(aggregate: Aggregate): CompletionStage<Done> =
            execute { em ->
                em.persist(aggregate)
                Done.getInstance()
            }

        override fun createAll(aggregates: Collection<Aggregate>): CompletionStage<Done> =
            execute { em ->
                aggregates.forEach { em.persist(it) }
                Done.getInstance()
            }

        override fun save(aggregate: Aggregate): CompletionStage<Done> =
            execute { em ->
                em.merge(aggregate)
                Done.getInstance()
            }

        override fun saveAll(aggregates: Collection<Aggregate>): CompletionStage<Done> =
            execute { em ->
